...
class BellDownSample(nn.Module):
    def bell_kernel(self, x):
        """
        ...
        abs_x = torch.abs(x)
        if abs_x <= 0.5:
            return .75 - torch.pow(abs_x, 2)
        elif 0.5 < abs_x < 1.5:
            return 0.5 * torch.pow(abs_x - 1.5, 2)
        else:
            return 0.0
    ...
    def __init__(self, factor=4):
        ...
        self.factor = factor
        size = factor * 4
        k = torch.tensor([self.bell_kernel((i - torch.floor(torch.tensor(size / 2)) + 0.5) / factor) for i in range(size)],dtype=torch.float32)
        k = k / torch.sum(k)
