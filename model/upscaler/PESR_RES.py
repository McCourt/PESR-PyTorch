...
from model.blocks import ChannelAttentionBlock, MeanShift, ConvolutionBlock, PixelShuffleUpscale, DepthSeparableConvBlock, ResBlock
...
class BasicBlock(nn.Module):
    def __init__(self, num_channel, res_scale=1, rep_pad=False):
        ...
        self.model_body = nn.Sequential(
            ResBlock(num_channel, basic_block=DepthSeparableConvBlock, rep_pad=rep_pad),
            ResBlock(num_channel, basic_block=DepthSeparableConvBlock, rep_pad=rep_pad),
            ResBlock(num_channel, basic_block=DepthSeparableConvBlock, rep_pad=rep_pad),
...
class PESR_RES(nn.Module):
    def __init__(self, scale=4, num_groups=12, num_channel=128, rep_pad=False):
        ...
        upscaler = [PixelShuffleUpscale(channels=num_channel,
                                        basic_block=ConvolutionBlock, rep_pad=rep_pad) for _ in range(int(log2(scale)))]
