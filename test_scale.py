    ...
    for scale in np.linspace(2, 4, num=5):
        for order_1 in range(6):
            for order_2 in range(6):
                mse = np.mean(np.square(resize(resize(hr, (h // scale, w // scale), mode='reflect', preserve_range=True, order=order_1), 
                                               (h, w), mode='reflect', preserve_range=True, order=order_2) - hr))
